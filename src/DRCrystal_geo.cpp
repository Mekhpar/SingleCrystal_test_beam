...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)  {
  ...
  //the assembled tower is a Stave
    ...
    // tower envelope
  dd4hep::Box towertrap(hwidth+tol,hwidth+0.1+0.4+tol,hzmax+0.1+0.4+tol);
  ...
  for(xml_coll_t li(x_det,_U(layer)); li; ++li)  
  {
	...
	// find top and bottom lengths at this position and center
      ...
      dd4hep::Box l_box(hwidth,hwidth+0.1+0.4,l_hzthick+0.1+0.4);
...
for(xml_coll_t si(x_layer,_U(slice)); si; ++si)  
{
	...
	/*dd4hep::VolCylinder s_cyl(hwidth,hwidth,s_hzthick);
	dd4hep::Volume     s_vol(s_name,s_cyl,description.material(x_slice.materialStr()));*/
...
//Left PMT
int s_num_1 = s_num; //Whatever this maximum value of s_num is after the previous loop, increment that so there is no overlap
//double y_bottoms2_1=hwidth;  
//int s_num_z_1 = s_num_y_3;
double z_bottom_2_1=-(l_hzthick+0.1+0.4);
for(xml_coll_t si_1(x_layer,_U(row)); si_1; ++si_1)  
{
	std::cout<<" with slice "<<s_num_1<<std::endl;
	xml_comp_t x_module_1 = si_1;
	string     s_name_1  = _toString(s_num_1,"slice%d");
	double     s_hzthick_1 = x_module_1.thickness()/2.;
	std::cout<<" with half  thickness "<<s_hzthick_1<<std::endl;

	double z_mid_2_1 = z_bottom_2_1 + s_hzthick_1;
	Position   s_pos(0.,0.,z_mid_2_1);      // Position of the slice (?)
	std::cout<<" placed at "<<z_mid_2_1<<std::endl;
	dd4hep::Tube s_tube(0.,1.1,s_hzthick_1,0.*deg,360*deg);


	dd4hep::Volume     s_vol(s_name_1,s_tube,description.material(x_module_1.materialStr()));
	DetElement slice(layer,s_name_1,det_id);
	...
	slice.setAttributes(description,s_vol,x_module_1.regionStr(),x_module_1.limitsStr(),x_module_1.visStr());
          ...
          // Slice placement.
	PlacedVolume slice_phv = l_vol.placeVolume(s_vol,s_pos);
	slice_phv.addPhysVolID("slice", s_num_1);
	slice.setPlacement(slice_phv);
          // Increment Z position of slice.

	z_bottom_2_1 += 2.*s_hzthick_1;
...
int s_num_z_1 = s_num_1; //Whatever this maximum value of s_num is after the previous loop, increment that so there is no overlap
//double y_bottoms2_1=hwidth;  
//int s_num_z_1 = s_num_y_3;
double z_bott2_1=l_hzthick;
for(xml_coll_t si_z_1(x_layer,_U(disk)); si_z_1; ++si_z_1)  
{
	...
	dd4hep::Box s_box(0.6/2,0.6/2,s_hzthick_z_1);


	dd4hep::Volume     s_vol(s_name_z_1,s_box,description.material(x_module_1.materialStr()));
...
double z_bott2_2=l_hzthick;
for(xml_coll_t si_z_2(x_layer,_U(envelope)); si_z_2; ++si_z_2)  
{
	...
	dd4hep::Box s_box(0.6/2,0.6/2,s_hzthick_z_2);


	dd4hep::Volume     s_vol(s_name_z_2,s_box,description.material(x_module_2.materialStr()));
...
for(xml_coll_t si_z_3(x_layer,_U(frame)); si_z_3; ++si_z_3)  
{
	...
	dd4hep::Box s_box(0.6/2,0.6/2,s_hzthick_z_3);


	dd4hep::Volume     s_vol(s_name_z_3,s_box,description.material(x_module_3.materialStr()));
...
for(xml_coll_t si_z_4(x_layer,_U(ladder)); si_z_4; ++si_z_4)  
{
	...
	dd4hep::Box s_box(0.6/2,0.6/2,s_hzthick_z_4);


	dd4hep::Volume     s_vol(s_name_z_4,s_box,description.material(x_module_4.materialStr()));
...
//Long face opposite to the incidence of the beam

//SiPM 1 + Silicone gap 1
int s_num_y_1 = s_num_z_4; //Whatever this maximum value of s_num is after the previous loop, increment that so there is no overlap
double y_bottoms2_1=hwidth;  
...
for(xml_coll_t si_y_1(x_layer,_U(shape)); si_y_1; ++si_y_1)  
{
	...
	Position   s_pos(-0.425,y_mids2_1,-0.425);      // Position of the slice (?)
...
for(xml_coll_t si_y_2(x_layer,_U(module)); si_y_2; ++si_y_2)  
{
	...
	Position   s_pos(-0.425,y_mids2_2,0.425);      // Position of the slice (?)
...
for(xml_coll_t si_y_3(x_layer,_U(chamber)); si_y_3; ++si_y_3)  
{
	...
	Position   s_pos(0.425,y_mids2_3,-0.425);      // Position of the slice (?)
...
for(xml_coll_t si_y_4(x_layer,_U(element)); si_y_4; ++si_y_4)  
{
	...
	Position   s_pos(0.425,y_mids2_4,0.425);      // Position of the slice (?)
